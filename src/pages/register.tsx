...
export default function RegisterPage() {
    ...
    const { isLoadingRegister, errorsResponse } = useSelector(userSelector)

    const [form, setForm] = useState<UserRegisterDTO>(new UserRegisterDTO())
    ...
    function onSubmit(e: ChangeEvent<HTMLFormElement>) {
        e.preventDefault();
        dispatch(userActions.registerNewUser(form))
    }
                    ...
                    <form onSubmit={onSubmit}>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-col">
                                <input onChange={onChange} value={form.fullname} className="border-2 border-emerald-500 rounded p-2 outline-0 bg-[#fafafa]" type="text" name="fullname" />
                                {!!errorsResponse['fullname'] && errorsResponse['fullname'].map(error => (
                        ...
                        <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 py-2 rounded mt-8 text-lg text-white font-semibold transition-all duration-300 flex items-center gap-2 justify-center">
                            Đăng ký <CircleSpinner size={20} loading={isLoadingRegister} />
                        </button>
                        ...
                        <button onClick={() => router.push(routes.login)} type="button" className="w-full border-2 border-emerald-600 py-2 rounded">
                            Đăng nhập
                        </button>
                    </form>
                </div>
